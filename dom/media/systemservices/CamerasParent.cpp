...
auto CamerasParent::RequestCameraAccess()
...
#if defined(WEBRTC_USE_PIPEWIRE)
  constexpr bool usePipewire = true;
#else
  constexpr bool usePipewire = false;
#endif

  if (!usePipewire ||
      !StaticPrefs::media_webrtc_camera_allow_pipewire_AtStartup()) {
  ...
  static RefPtr<CameraAccessRequestPromise> sCameraAccessRequestPromise;
  if (!sCameraAccessRequestPromise) {
    auto options = MakeRefPtr<VideoCaptureOptionsImpl>();
    auto portal = std::make_unique<CameraPortalImpl>();
    sCameraAccessRequestPromise =
        portal->Start()
            ->Then(
                GetCurrentSerialEventTarget(),
                "CamerasParent::RequestCameraAccess portal handler",
                [options, portal = std::move(portal)](int aFD) {
                  return options->Init(aFD);
                },
                [](nsresult aRv) {
    ...
    static bool clearingSetup = ([&] {
      ClearOnShutdown(&sCameraAccessRequestPromise);
      return true;
    }());
  }
  ...
  return sCameraAccessRequestPromise->Then(
      ...
      [promise = sCameraAccessRequestPromise](nsresult aRv) {
